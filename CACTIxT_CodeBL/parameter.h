 ...
 public:
  class DeviceType
  {
   public:
    ...
    double R_nch_on;
    double R_pch_on;
    double S_Rn; // Ron sensitivity vs. Vth for NMOS CMOS device (Wn=1um)
    double S_Rp; // Ron sensitivity vs. Vth for PMOS CMOS device (Wp=1um)
    double Vdd;
    double Vth;
    double sigma_vth; // for sensitivity
    double I_on_n;
    double I_on_p;
    double I_off_n;
    double I_off_p;
    ...
    DeviceType(): P_fin(0), H_fin(0), T_si(0), C_g_ideal(0),
                  ...
                  l_phy(0), l_elec(0), R_nch_on(0), R_pch_on(0), S_Rn(0), S_Rp(0), Vdd(0), Vth(0), sigma_vth(0),
  ...
  class MemoryType
  {
   public:
    ...
    double cell_readacc_w; //  width of read access transistors for 10T cell: AL2 e AR2
    double cell_acc_w;     //  width of access transistors for 10T cell: AL1 e AR1
    double cell_a_w;      // Alireza: width of (write) access transistors for 6T and 8T cells
    double cell_pmos_w;   // Alireza: width of pull-up (pmos) transistors for 6T and 8T cells - () 10T as well
    double cell_nmos_w;   // Alireza: width of pull-down (nmos) transistors for 6T and 8T cells - () 10T as well
    ...
    double cell_iso_w;    //  width of the isolator transistor for 10T cell -> NL e NR
  ...
  double w_sense_n;
  double w_sense_p;
  // start -- defining the sizing of the skewed inverter for 8T sensing
  double w_skewed_inv_n;
  double w_skewed_inv_p;
  //  end
  double sense_delay;
