    ...
    bool is_main_mem)
{
  ...
  else
  {
    ...
    ptr_array->row_decoder_writing_delay    = uca->bank.mat.row_dec->delay;  // save the delay of decoder, demux (if any) and WL for 6T (or WWL for 8T and 10T)
    ptr_array->row_decoder_reading_delay    = uca->bank.mat.row_dec->delay_rd; // save the delay of decoder, demux and RWL (for 8T and 10T)
    ptr_array->sigma_delay_wl_driver        = uca->bank.mat.row_dec->sigma_delay_wl_driver; // save the STD of the WL/WWL driver delay
    ptr_array->sigma_delay_rwl_driver       = uca->bank.mat.row_dec->sigma_delay_rwl_driver; // save the STD of the RWL driver delay
    ptr_array->delay_bitlines               = uca->bank.mat.delay_bitline; // save the reading delay of the bitline
    ...
    //ptr_array->power_bitlines.readOp.dynamic = uca->bank.mat.power_bitline.readOp.dynamic;

    //cout<<"ptr_array->power_bitlines.readOp.dynamic: "<<ptr_array->power_bitlines.readOp.dynamic<<endl;
    ptr_array->power_bitlines.readOp.leakage = uca->bank.mat.power_bitline.readOp.leakage * dyn_p.num_act_mats_hor_dir;    //
    ptr_array->power_sense_amps.readOp.dynamic = uca->bank.mat.power_sa.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
    ptr_array->power_decoder.readOp.dynamic = uca->bank.mat.power_decoder.readOp.dynamic * dyn_p.num_act_mats_hor_dir;     //
    ptr_array->power_wordline.readOp.dynamic = uca->bank.mat.power_wordline.readOp.dynamic * dyn_p.num_act_mats_hor_dir;   //
    ptr_array->power_wordline.writeOp.dynamic = uca->bank.mat.power_wordline.writeOp.dynamic * dyn_p.num_act_mats_hor_dir; //
